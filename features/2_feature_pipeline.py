"""
AQI Feature Pipeline - Your Original Architecture
==================================================
Features are engineered DURING ingestion (your preferred approach)
With critical fixes: rolling mean leakage + timezone handling
"""
...
# Rolling Trends - CRITICAL FIX: shift(1) before rolling to prevent data leakage
df['pm25_rolling_mean_24h'] = df['pm25'].shift(1).rolling(window=24, min_periods=1).mean()
df['pm25_rolling_std_24h'] = df['pm25'].shift(1).rolling(window=24, min_periods=1).std()
